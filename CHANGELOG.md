...
### Fixed
- `-Restore` puts back the DNS recorded in the newest backup for the adapter instead of always resetting to DHCP. `Backup-DnsSettings` had been writing `dns-backup_*.json` since the first release and nothing ever read one, so anyone running a Pi-hole, a work resolver, or any other static config had it saved and then dropped, and the tool said the restore worked. `Get-LatestDnsBackup` skips files from other adapters and files it cannot parse; `Get-DnsRestorePlan` decides between a static restore and a DHCP reset and explains which it chose. A static restore resets the adapter first, since `Set-DnsClientServerAddress` infers the address family per address, so an IPv6 pair from an earlier `-IncludeIPv6` run would otherwise survive a restore of an IPv4-only backup. Falls back to the old behaviour when there is no backup or the backup shows DHCP, and the new `-ResetDhcp` switch forces that reset on demand. The DNS cache is flushed instead of telling you to run `ipconfig /flushdns` yourself.
- The "you're already using the best DNS" shortcut compared only `$currentDns[0]` against the winner's primary. It predated IPv6 support, so a machine already on the winning IPv4 pair with the router's IPv6 DNS still bound counted as done and `-IncludeIPv6` exited without applying anything, which is the exact dual-stack leak the switch exists to close. It also waved through a mismatched secondary and an adapter carrying a third server. `Test-DnsAlreadyOptimal` now weighs both addresses, and both families when `-IncludeIPv6` is in play.
- `Get-ActiveNetworkAdapter` took the first row `Get-NetAdapter` returned, and that order is neither documented nor stable, so on a machine with Ethernet and Wi-Fi both up the adapter that got benchmarked and written to was luck. Selection now prefers the candidate holding the default route, cheapest `RouteMetric` plus `InterfaceMetric` first, with ties going to the earlier adapter and an unreadable route table falling back to the old behaviour. The ranking sits in `Select-PreferredAdapter`, which takes adapters and routes as arguments so it can be tested offline.
...
- `-ResetDhcp`, used with `-Restore`, resets the adapter to automatic DNS and ignores the backup. This is the old unconditional `-Restore` behaviour, kept for anyone who wants it.
- Pester coverage for the three fixes above: `Test-DnsAlreadyOptimal` across both families and the partial-match cases, `Get-LatestDnsBackup` and `Get-DnsRestorePlan` including a malformed backup file and a round trip through `Backup-DnsSettings`, and `Select-PreferredAdapter` across route ranking, ties, and every fallback. 79 tests to 118.
